...
echo [%date% %time%] start %* >> F:\pubmed\logs\booknote_daily.log
%PY% -u harvest\daily.py %* >> F:\pubmed\logs\booknote_daily.log 2>&1
set RC=%errorlevel%
echo [%date% %time%] end rc=%RC% >> F:\pubmed\logs\booknote_daily.log
if not %RC%==0 echo [%date% %time%] BAT_FAIL rc=%RC% >> F:\pubmed\logs\wos_problems.log
